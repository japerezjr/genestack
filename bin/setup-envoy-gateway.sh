...
create_gateway() {
    ...
    for gw_type in $gateway_types; do
        if [[ "$gw_type" == "external" ]]; then
            ...
            local http_port=$((8080 + port_offset))
            local https_port=$((8443 + port_offset))
        else
...
process_gateway_routes() {
    ...
    for route in $routes_list; do
        local route_file="/opt/genestack/etc/gateway-api/routes/custom-${route}-gateway-route.yaml"
        if [[ -f "$route_file" ]]; then
            ...
            for gw_type in $gateway_types; do
                ...
                # Update the route name to be unique
                sed -i "s/name: ${route}/name: ${route}-${gw_type}/g" "/tmp/${route}-${gateway_name}-${gw_type}.yaml"
...
process_gateway_listeners() {
    ...
    for gw_type in $gateway_types; do
        ...
        for route in $routes_list; do
            local listener_file="/opt/genestack/etc/gateway-api/listeners/${route}-https.json"
            if [[ -f "$listener_file" ]]; then
                local output_file="/etc/genestack/gateway-api/listeners/${gateway_name}/${route}-${gw_type}-https.json"
                sed "s/your.domain.tld/${domain}/g" "$listener_file" > "$output_file"
                echo "  - Processed listener: $route for $gw_type gateway"
            else
                echo "  - Warning: Listener file not found for $route"
            fi
        done
    ...
    while IFS='|' read -r gw_name gw_namespace gw_domain gw_types external_pool internal_pool issuer_type issuer_email challenge dns_plugin api_token api_key routes; do
        if [[ -n "$gw_name" ]]; then
            ...
            for gw_type in $gw_types; do
                local gw_instance_name="${gw_name}-${gw_type}"
